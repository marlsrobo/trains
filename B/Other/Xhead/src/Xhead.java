...
/*
    ...
    - Write tests with numLines 0, >0, <0, missing (no args), and malformed ("-asdf", "asdf", "--2").
    - Write tests with numLines < the number of lines in STDIN
    # Check all the project deliverables
        - Readme
        - Folders
    # Make sure it runs of Linux machines
 */
  ...
  private static void printLines(int numLines) {
    ...
    for (int i = 0; i < numLines; i++) {
      if (scanner.hasNextLine()){     // check if there is a new line to print
        System.out.println(scanner.nextLine());  // print every line
      }
      else {
        return;
      }
